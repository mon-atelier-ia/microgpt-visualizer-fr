...
import type { InferenceStep } from "../engine/model";
...
export const GeneratePanel = memo(function GeneratePanel({
  untrained,
  onGenerate,
  onClear,
  temperature,
  ...
  return (
    <div className="panel">
      ...
      <div className="controls">
        <button type="button" className="btn" onClick={() => onGenerate(1)}>
          Générer 1
        </button>
        <button
          type="button"
...
interface GeneratedNamesProps {
  ...
  activeTrace: GeneratedResult | null;
  onSelect: (r: GeneratedResult) => void;
}

export const GeneratedNames = memo(function GeneratedNames({
  results,
  activeTrace,
  onSelect,
}: GeneratedNamesProps) {
...
/* ── Generation trace (step-by-step + probabilities) ─ */

function computeTop12(step: InferenceStep | undefined) {
  if (!step) return [];
  return step.probs
...
interface GenerationTraceProps {
  trace: GeneratedResult;
  activeStep: number;
  onStepChange: (i: number) => void;
}

export const GenerationTrace = memo(function GenerationTrace({
  trace,
  activeStep,
  onStepChange,
}: GenerationTraceProps) {
  const step = trace.steps[activeStep];
  const top10 = useMemo(() => computeTop12(step), [step]);
    ...
    <div className="panel-row">
      <div className="panel">
        <div className="panel-title">
          Trace de génération : « {trace.name} »
        </div>
        <div className="explain">
          Clique sur chaque étape pour voir ce que le modèle « pensait » à cette
          position. Le modèle pioche dans la <Term id="distribution" /> de
        ...
        {step && (
          <div className="trace">
            {trace.steps.map((s, i) => (
              <div key={s.pos} style={{ opacity: i === activeStep ? 1 : 0.5 }}>
      ...
      <div className="panel">
        ...
        <div className="explain">
          Le modèle produit ces probabilités pour le caractère suivant. Celui en{" "}
          <span className="text-green">vert</span> a été sélectionné par{" "}
...
/* ── How inference works explanation ───────────────── */

export const HowInferenceWorks = memo(function HowInferenceWorks() {
  return (
    <div className="panel">
      ...
      <div className="explain">
        ...
        <br />
        <b>4.</b> Si le token échantillonné est <Term id="bos" /> → arrêter (fin
        du nom).
        <br />
        <b>5.</b> Sinon, renvoyer le token échantillonné en entrée et reprendre
        à l'étape 2.
        <br />
        <br />
        ...
        tokens précédents. C'est exactement ainsi que fonctionne ChatGPT, juste
