...
describe("valToColor", () => {
  ...
  it("v=0 retourne couleur neutre", () => {
    const result = valToColor(0, { alpha: 1, green, red, neutral });
  ...
  it("v=1 retourne couleur verte", () => {
    const result = valToColor(1, { alpha: 0.8, green, red, neutral });
    expect(result).toMatch(/^rgba\(/);
    const m = result.match(/rgba\((\d+),(\d+),(\d+),([\d.]+)\)/);
    expect(m).not.toBeNull();
  ...
  it("v=-1 retourne couleur rouge", () => {
    const result = valToColor(-1, { alpha: 1, green, red, neutral });
    expect(result).toMatch(/^rgba\(/);
  });

  it("clamp v > 1", () => {
    const a = valToColor(1, { alpha: 1, green, red, neutral });
    const b = valToColor(5, { alpha: 1, green, red, neutral });
    expect(a).toBe(b);
  });
  ...
  it("alpha is included in output", () => {
    const result = valToColor(0.5, { alpha: 0.3, green, red, neutral });
    expect(result).toContain(",0.3)");
  });

  it("produces valid RGB values (0-255)", () => {
    for (const v of [-1, -0.5, 0, 0.5, 1]) {
      const result = valToColor(v, { alpha: 1, green, red, neutral });
